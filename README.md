...
│   └── smartwatch/                  # Package generico multi-dispositivo
│       ├── __init__.py              # Entry point pubblico + registry per marca
│       ├── xiaomi.py                # Parser Xiaomi/Redmi/Poco (attività + sonno)
│       ├── amazfit.py               # Placeholder per supporto futuro
│       ├── garmin.py                # Placeholder per supporto futuro
│       └── huawei.py                # Placeholder per supporto futuro
│
...
1. **Aggiorna i dati** — lancia `python trackingProgress/updateTracking.py` e usa il menu per inserire nuove misure corporee, registrare una nuova fase dieta, importare gli allenamenti, oppure sincronizzare lo smartwatch. Quest'ultima opzione legge `Gadgetbridge.db`, elabora i dati (attività e sonno) e aggiorna `dashboardData.db`, insieme a una copia coerente dei dati correnti presenti nell'Excel.
...
| **Smartwatch** | Passi, frequenza cardiaca, SpO2, stress, calorie e sonno, con selettore Giorno/Settimana/Mese e drill-down sui dati al minuto per una giornata specifica |
...
- [x] Lettura dati smartwatch da Gadgetbridge (famiglia Xiaomi, non ancora persistiti)
- [ ] Refactor di `smartwatch.py` in package generico multi-marca
- [ ] Estrazione dati del sonno
- [ ] Motore di sincronizzazione verso `dashboardData.db`
- [ ] Applicazione web Flask (routes, template, grafici)
